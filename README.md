...
- **增量扫描**：媒体库（识别结果）持久化到数据目录，启动即加载、页面立即可用；扫描只处理新增/变化的文件，已识别视频保留探测结果（probe 缓存）不再重复探测。设置页提供「重置媒体库」按钮，点击后清空媒体库与探测缓存，才执行真正的全量重新扫描。
...
| POST | `/api/library/reset` | 重置媒体库：清空媒体库与探测缓存，触发全量重新扫描 |
